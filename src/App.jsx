...
`;
  ...
  if (screen === 'home')
    return (
      <Homepage onLogin={() => setScreen('login')} onRegister={() => setScreen('register')} />
    );
  if (screen === 'login')
    return (
      <Login users={users} onLogin={login} onRegister={() => setScreen('register')} />
    );
  if (screen === 'register')
    return (
      <Register
        ...
        onSuccess={(u) => { addPending(u); setScreen('pending'); }}
      />
    );
  if (screen === 'pending')
  ...
  const SidebarContent = () => (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '14px 10px', flex: 1, overflow: 'auto' }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: '#334155', textTransform: 'uppercase', letterSpacing: '1px', padding: '0 8px', marginBottom: 8 }}>
          Menu
        </div>
        {navItems.map((n) => (
          <div
          ...
          >
            ...
            {n.id === 'admin' && pending.length > 0 && (
              <span style={{ marginLeft: 'auto', width: 18, height: 18, borderRadius: '50%', background: T.amber, color: '#fff', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {pending.length}
              </span>
            )}
          </div>
        ))}
        <div style={{ fontSize: 10, fontWeight: 700, color: '#334155', textTransform: 'uppercase', letterSpacing: '1px', padding: '12px 8px 8px', marginTop: 4 }}>
          Recenti
        </div>
        {sharedHistory.length === 0 && (
          <div style={{ fontSize: 11, color: '#334155', padding: '4px 12px', fontStyle: 'italic' }}>
        ...
        {sharedHistory.slice(0, 8).map((h) => (
          ...
          >
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1 }}>{h.title}</div>
            </div>
  ...
  // Larghezza massima per pagina
  const getMaxWidth = () => {
    if (page === 'gantt') return '100%';
    if (page === 'dashboard') return 1400;
    if (['ranking', 'chat_docs', 'chat_ai'].includes(page)) return 1100;
    return 900;
  };
  ...
  return (
    <div style={{ fontFamily: "'Inter',-apple-system,sans-serif", minHeight: '100vh', background: T.surfaceAlt, color: T.text, display: 'flex' }}>
      {!mob && (
        <div style={{ width: 236, background: T.sidebar, minHeight: '100vh', flexShrink: 0, borderRight: '1px solid ' + T.sidebarBorder }}>
          <SidebarContent />
        </div>
      )}
      {mob && sidebarOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex' }}>
          <div style={{ width: 236, background: T.sidebar, height: '100vh', borderRight: '1px solid ' + T.sidebarBorder }}>
            <SidebarContent />
          </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: mob ? 14 : '26px 32px' }}>
          <div style={{ maxWidth: getMaxWidth(), margin: '0 auto', width: '100%' }}>
            ...
            {page === 'admin' && (user.role === 'admin' ? (
              <AdminUsers users={users} pending={pending} onApprove={approve} onReject={reject} />
            ) : (
              <AccessDenied />
            ))}
          </div>
